...
use openapiv3::{OpenAPI, Operation, Parameter, ParameterKind, ReferenceOr};
...
impl<Router: Default> Server<Router, OpenAPI> {
    ...
    fn add_handler_to_spec<F>(&mut self, path: &str, method: Method, _handler: &F) {
        ...
        let mut operation = OPERATION_LOOKUP.get(type_name).expect(&format!(
    ...
    pub fn swagger_ui(mut self, swagger_ui_route: &str) -> Self {
        if !swagger_ui_route.ends_with('/') {
            panic!(
                "Swagger UI route must end with a slash. Without it, static resources will not be found."
            );
        }
